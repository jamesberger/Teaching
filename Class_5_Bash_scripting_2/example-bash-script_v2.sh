...
# 1. When in doubt, echo it out.
# 2. bash –n myscript.sh – check for syntax errors, but don’t run script
# 3. Bash –v myscript.sh – echoes each command before running it (verbose mode)
# 4. Check for syntax errors and run script with verbose output – bash –nv myscript.sh
# 5. Break things into smaller chunks to help pinpoint where something is failing. ]
# (finding one error in a single 500 line statement vs finding one error in ten 50 line statements).
# This is covered in more detail when we get into writing functions.
